...
@Component({
  selector: 'app-resume-strategy-dialog',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  template: `
    ...
    <mat-dialog-content>
      <form [formGroup]="resumeStrategyForm" class="resume-strategy-form">
        <mat-form-field appearance="outline" class="full-width">
          <mat-label>Display Name</mat-label>
          <input matInput formControlName="displayName" placeholder="Enter resume strategy display name" />
          <mat-icon matPrefix>title</mat-icon>
          <mat-error *ngIf="resumeStrategyForm.get('displayName')?.hasError('required')">
            Display name is required
          </mat-error>
          <mat-error *ngIf="resumeStrategyForm.get('displayName')?.hasError('minlength')">
            Display name must be at least 3 characters
          </mat-error>
          <mat-error *ngIf="resumeStrategyForm.get('displayName')?.hasError('maxlength')">
        ...
        <mat-form-field appearance="outline" class="full-width">
          ...
          <mat-error *ngIf="resumeStrategyForm.get('actualValue')?.hasError('required')">
            Actual value is required
          </mat-error>
          <mat-error *ngIf="resumeStrategyForm.get('actualValue')?.hasError('minlength')">
            Actual value must be at least 1 character
          </mat-error>
          <mat-error *ngIf="resumeStrategyForm.get('actualValue')?.hasError('maxlength')">
        ...
        <mat-form-field appearance="outline" class="full-width">
          ...
          <textarea
            ...
            placeholder="Enter resume strategy description (optional)"
            rows="3"
          ></textarea>
          <mat-icon matPrefix>description</mat-icon>
          <mat-error *ngIf="resumeStrategyForm.get('description')?.hasError('maxlength')">
...
export class ResumeStrategyDialogComponent implements OnInit {
  resumeStrategyForm!: FormGroup;

  constructor(
    private fb: FormBuilder,
    public dialogRef: MatDialogRef<ResumeStrategyDialogComponent>,
    @Inject(MAT_DIALOG_DATA) public data: ResumeStrategyDialogData
  ) {}
  ...
  private populateForm(resumeStrategy: ResumeStrategyDisplayData): void {
    this.resumeStrategyForm.patchValue({
  ...
  onSubmit(): void {
    if (this.resumeStrategyForm.valid) {
      const formValue = this.resumeStrategyForm.value;
      const request: ResumeStrategyCreateRequest | ResumeStrategyUpdateRequest = {
